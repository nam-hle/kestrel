...
target TypeScript repo — one *prefers* the `symantic` CLI but may fall back to grep/Read, one
...
- **One fixed question, reused across rounds/repos.** The same question is asked every round so
  the experiment tracks symantic *converging to grep parity over time*: a gap found in round N
...
question; only override to start a *new* tracking series.
...
4. **Verify each candidate gap** — reproduce it on **symantic's own fixtures**
   ...
   (a fallback the agent took out of habit, where a symantic op would have worked, is *not* a
...
- **Verify before filing.** Reproduce a gap (or drop it). A grep fallback is a *lead*, not a
...
> 2. **TOOLING LOG**: numbered list of every command — symantic *and* grep/Read — each with a
